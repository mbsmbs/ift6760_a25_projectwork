...
class RDKitConformer:
    def __init__(self, atom_positions, smiles, freely_rotatable_tas=None, freely_variable_bonds=None, freely_variable_angles=None,):
        """
        ...
        self.freely_variable_bonds = freely_variable_bonds
        self.freely_variable_angles = freely_variable_angles
    ...
    def set_bond_length(self, bond, value):
        i, j = bond
        rdMolTransforms.SetBondLength(self.rdk_conf, int(i), int(j), float(value))

    def get_bond_angle(self, angle_atoms):
        i, j, k = angle_atoms
        return rdMolTransforms.GetAngleRad(self.rdk_conf, int(i), int(j), int(k))

    def set_bond_angle(self, angle_atoms, value):
        i, j, k = angle_atoms
        rdMolTransforms.SetAngleRad(self.rdk_conf, int(i), int(j), int(k), float(value))

    def get_variable_bond_lengths(self):
        return [self.get_bond_length(b) for b in self.variable_bonds]

    def get_variable_bond_angles(self):
        return [self.get_bond_angle(a) for a in self.variable_angles]
